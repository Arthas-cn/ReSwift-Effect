...
struct TestValueStringReducer {
    func handleAction(state: inout TestStringAppState, action: Action, environment: TestEnvironment) -> Task<Action, Error>? {
        if state.testValue.isEmpty {
...
struct TestCustomAppStateReducer {
    func handleAction(state: inout TestCustomAppState, action: Action, environment: TestEnvironment) -> Task<Action, Error>? {
        if state.substate.value == 0 && state.substate.value == 0 {
            state = TestCustomAppState()
        }
...
struct TestNonEquatableReducer {
    func handleAction(state: inout TestNonEquatable, action: Action, environment: TestEnvironment) -> Task<Action, Error>? {
        if state.testValue.testValue.isEmpty {
            state = TestNonEquatable()
        }
...
class DispatchingSubscriber: StoreSubscriber {
    ...
    init(store: Store<TestAppState, TestEnvironment>) {
        self.store = store
    }
